...
このとき `"done"` が出力された後もクエリのログは出力されず、レコードも作成されません。`findMany` や `$executeRaw` など、DB 問い合わせが発生する他のクエリでも同様です。なぜこのようなことが起こるのか、説明する前にまず Thenable オブジェクトについて知っておく必要があります。
